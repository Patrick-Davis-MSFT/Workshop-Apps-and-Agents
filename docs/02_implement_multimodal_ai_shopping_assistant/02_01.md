...
> Note you do not need to update the `chat_app.py` file just *uncomment* lines of code
...
api_key = os.getenv("gpt_api_key") or os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
```
...
```python
# Initialize Azure OpenAI client for GPT model using API key
if not api_key:
    raise ValueError("Missing API key. Set gpt_api_key, AZURE_OPENAI_API_KEY, or OPENAI_API_KEY.")
