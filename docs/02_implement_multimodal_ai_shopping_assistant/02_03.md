...
{: .note }
> If you receive a permissions error, ensure that the Container App's managed identity has the **AcrPull** and **Container Registry Repository Reader** role on your Azure Container Registry. The Bicep template should have configured this, but you can verify it in the **Access control (IAM)** blade of the Container Registry in the Azure portal. This needs to be done on the App Service Enviroment. This take 5 minutes to take affect.
...
```bash
az containerapp update --name {YOUR_CONTAINER_APP_NAME} --resource-group {YOUR_RESOURCE_GROUP} --set-env-vars FOUNDRY_ENDPOINT="{YOUR_FOUNDRY_ENDPOINT}" gpt_endpoint="{YOUR_GPT_ENDPOINT}" "embedding_endpoint="{YOUR_EMBEDDING_ENDPOINT}" phi_4_endpoint="{YOUR_PHI4_ENDPOINT}" FOUNDRY_API_KEY="{YOUR_FOUNDRY_API_KEY}" gpt_api_key="{YOUR_GPT_5_4_KEY}"
```
