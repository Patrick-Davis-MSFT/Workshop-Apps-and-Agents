...
#### Using Azure Developer CLI Perfered

If you prefer to use the Azure Developer CLI, you can run the following command in your terminal:

```bash
azd down --purge
```

> Note: In Real development enviroments you should seperate the AI / Enterprise Infrastructure from the application infrastructure (In this hack, you would want to use a azure foundry hub and the models created outside the Azure Developer Command Line ). The reason for doing this is to allow azd up / down to be run every day and not affect shared services. 
